...
class PluginValidationWorker:
    ...
    async def process_job(self, job_id: UUID) -> None:
        async with AsyncSessionFactory() as session:
            ...
            try:
                version = await self._resolve_version(session, job)
                ...
                if actual_checksum.lower() != version.checksum_sha256.lower():
                ...
                final_path = self.validated / job.plugin_key / version.version
    ...
    async def _resolve_version(
        ...
        if item_id is None:
            raise PackageValidationError("published Marketplace plugin not found")

        statement = select(PluginMarketplaceVersion).where(
            PluginMarketplaceVersion.marketplace_item_id == item_id
        )
        if job.requested_version:
            statement = statement.where(
                PluginMarketplaceVersion.version == job.requested_version
            )
        else:
            statement = statement.order_by(
                PluginMarketplaceVersion.released_at.desc()
            )
        version = (await session.execute(statement.limit(1))).scalar_one_or_none()
        if version is None:
            raise PackageValidationError("requested Marketplace version not found")
        return version
    ...
    def _download_package(
        ...
        if parsed.scheme != "https":
            raise PackageValidationError("only HTTPS package URLs are allowed")
        if not parsed.hostname:
            raise PackageValidationError("package URL has no hostname")
        if parsed.username or parsed.password:
            raise PackageValidationError("credentials in package URLs are forbidden")
        ...
        request = urllib.request.Request(
            package_url,
            headers={"User-Agent": "ShieldNet-Plugin-Validator/1.0"},
        )
        ...
        try:
            with urllib.request.urlopen(
                ...
                if content_length and int(content_length) > self.settings.max_package_bytes:
    ...
    async def _record_validated(
        ...
        session.add(
            PluginInstallLog(
                job_id=job.id,
                level="info",
                message="package validated and prepared; no plugin code was executed",
                metadata_json={"path": str(final_path), "version": version},
            )
        )
        session.add(
            PluginRuntimeEvent(
                ...
                metadata_json={"version": version, "path": str(final_path)},
            )
        )
        await session.commit()
