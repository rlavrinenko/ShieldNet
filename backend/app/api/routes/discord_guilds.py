...
async def list_my_guilds(current_user:User=Depends(get_current_user),session:AsyncSession=Depends(get_db_session)):
    ...
    return [GuildAccessResponse(guild_id=str(g.guild_id),name=g.name,icon_url=g.icon_url,owner_discord_id=str(g.owner_discord_id),member_count=g.member_count,guild_status=g.status.value,bot_status=g.bot_status.value,access_role=m.role.value) for g,m in result.all()]
