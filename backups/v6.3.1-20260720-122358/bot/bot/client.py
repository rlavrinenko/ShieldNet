...
from bot.translator import TranslatorClient, translation_embed
...
class ShieldNetBot(discord.Client):
    def __init__(self) -> None:
        ...
        self.translator = TranslatorClient(self.backend)
    ...
    def _register_commands(self) -> None:
        ...
        @app_commands.describe(
            ...
            target_language="Target language code, for example en, uk, de or ja",
        ...
        async def translate_cmd(
            ...
            if not target or len(target) > 32 or len(source) > 32:
                await interaction.response.send_message(
                    "Use a valid language code such as en, uk, de, fr, ja or auto.",
                    ephemeral=True,
                )
                return
            ...
            try:
                ...
                logger.exception("Translation failed for guild=%s user=%s", interaction.guild.id, interaction.user.id)
                await interaction.followup.send(
                    self.translator.user_error_message(exc),
                    ephemeral=True,
                )
                return

            await interaction.followup.send(
                embed=translation_embed(result, source, target),
                ephemeral=private,
            )
